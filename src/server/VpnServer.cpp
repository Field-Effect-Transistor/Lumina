...
void VpnServer::startVpnServer() {
    ...
    std::string command = "systemctl start /etc/systemd/system/openvpn-" + sanitize(m_server_name) + ".service";
...
void VpnServer::restartVpnServer() {
    ...
    std::string command = "systemctl restart /etc/systemd/system/openvpn-" + sanitize(m_server_name) + ".service";
    auto result = execute(command);
    if (result.exit_code != 0) {
...
void VpnServer::stopVpnServer() {
    ...
    std::string command = "systemctl stop /etc/systemd/system/openvpn-" + sanitize(m_server_name) + ".service";
    auto result = execute(command);
    if (result.exit_code != 0) {
